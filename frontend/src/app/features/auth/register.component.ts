...
@Component({
    ...
    <section class="auth-page">
      ...
      <div class="auth-desktop">
        <div class="auth-layout">
          <aside class="auth-showcase" aria-hidden="true">
            ...
            <div class="showcase-copy"><span class="eyebrow">CV CREATOR</span><h1>Your next chapter starts here.</h1><p>Create a polished CV in minutes, then make every application count.</p></div>
            <a routerLink="/login" class="showcase-switch">Already a member? <strong>Sign in</strong></a>
          </aside>
          <main class="auth-card">
            <a routerLink="/" class="back-link"><span>‹</span> Back to home</a>
            <div class="auth-heading"><p class="eyebrow">GET STARTED</p><h2>Create your account</h2><p>Start building your professional CV today.</p></div>
            <form [formGroup]="form" (ngSubmit)="submit()">
              <label>Full name<input formControlName="fullName" type="text" placeholder="Your full name" autocomplete="name" /></label>
              <label>Email address<input formControlName="email" type="email" placeholder="you@example.com" autocomplete="email" /></label>
              <label>Password
                <div class="password-wrap">
                  <input formControlName="password" [type]="showPw() ? 'text' : 'password'" placeholder="At least 8 characters" autocomplete="new-password" />
                  <button type="button" class="eye-btn" (click)="showPw.set(!showPw())">
                    @if (showPw()) { <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
              ...
              <label class="check-label terms"><input type="checkbox" /> <span>I agree to the <a href="#">Terms of Service</a></span></label>
              @if (error()) { <p class="form-error">{{ error() }}</p> }
              <button type="submit" [disabled]="form.invalid || loading()">Create account <span>→</span></button>
            ...
            <p class="account-link">Already have an account? <a routerLink="/login">Sign in</a></p>
          </main>
        </div>
      </div>
      ...
      <div class="auth-mobile">
        ...
        <div class="mobile-card">
          <div class="m-tabs">
            <a routerLink="/login" class="m-tab">Sign in</a>
            <a routerLink="/register" class="m-tab active">Sign up</a>
          </div>
          <form [formGroup]="form" (ngSubmit)="submit()">
            <label>Full Name<input formControlName="fullName" type="text" placeholder="Enter Full Name" autocomplete="name" /></label>
            <label>Email<input formControlName="email" type="email" placeholder="Enter Email" autocomplete="email" /></label>
            <label>Password
              <div class="password-wrap">
                <input formControlName="password" [type]="showPw() ? 'text' : 'password'" placeholder="Enter Password" autocomplete="new-password" />
                <button type="button" class="eye-btn" (click)="showPw.set(!showPw())">
            ...
            <label class="check-label terms"><input type="checkbox" /> <span>I agree to the processing of <a href="#">Personal data</a></span></label>
            @if (error()) { <p class="form-error">{{ error() }}</p> }
  ...
  styles: [`
    ...
    form label:not(.check-label){display:block;font-size:.76rem;font-weight:700;color:#45516a;margin:0 0 14px}input:not([type=checkbox]){box-sizing:border-box;display:block;width:100%;padding:12px 13px;margin-top:7px;border:1px solid #dfe4ef;border-radius:9px;color:#1d2b4d;font:inherit;font-size:.87rem;outline:none;transition:.2s}input:not([type=checkbox]):focus{border-color:#4c70d5;box-shadow:0 0 0 3px #dce6ff}
    .check-label{display:flex;gap:7px;align-items:center;color:#788397;font-size:.73rem}.check-label input{accent-color:#4268ca}.terms{margin:2px 0 20px}.terms a{color:#4167ca;font-weight:700;text-decoration:none}.form-error{font-size:.8rem;color:#c1354d;margin:-8px 0 15px}
...
export class RegisterComponent {
  ...
  constructor(private fb: FormBuilder, private auth: AuthService, private router: Router, private toast: ToastService) { this.form = this.fb.group({ fullName: ['', Validators.required], email: ['', [Validators.required, Validators.email]], password: ['', [Validators.required, Validators.minLength(8)]] }); }
  submit() { if (this.form.invalid) return; this.loading.set(true); this.error.set(null); const { fullName, email, password } = this.form.getRawValue(); this.auth.register(fullName!, email!, password!).subscribe({ next: () => { this.loading.set(false); this.toast.success('Account created!'); this.router.navigate(['/my-cv']); }, error: err => { this.loading.set(false); this.error.set(err.error?.message || 'Registration failed.'); } }); }
}
